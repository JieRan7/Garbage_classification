...
colors_list = [(139, 0, 139), 
               ...
               (100,226,127)]
"""
def press_any_key_exit(msg='Press any key to continue...'):
...
def yolov3_get_txt(dir,label_list):
    ...
    for filename in tqdm(trainList):
        ...
        roots = tree.getroot()
        image_path = os.path.join(dir , "train", "JPEGImages", str(filename.split('.xml')[0])+".jpg")
        image = cv2.imread(image_path)
    ...
    for filename in tqdm(testList):
        ...
        tree = ET.parse(xmlname)
        roots = tree.getroot()
        image_path = os.path.join(dir , "test","JPEGImages" , str(filename.split('.xml')[0]) + ".jpg")
        image = cv2.imread(image_path)
        if image is None:
...
def yolov3_move_image(dir):
    ...
    train_images_path = os.path.join(dir, "train", 'JPEGImages')
    
    test_images_path = os.path.join(dir , "test","JPEGImages")
...
def yolov3_get_train_test_file(dir,val_percent):
    ...
    train_JPEGImages = os.path.join(train_file_path , "JPEGImages")
    ...
    for filename in images_label_file_List:
        ...
        for xml_name in xmls_List:
            ...
            train_xml_path = os.path.join(train_Annotations, xml_name)
            test_xml_path = os.path.join(test_Annotations, xml_name)
        ...
        for i in tqdm(range(len(xmls_List))):
            ...
            if i <= val_number:
                ...
                # 修改后（直接使用原名）
                xml_save_path = os.path.join(test_Annotations, xmls_List[i])
            else:
                # xml_save_path = os.path.join(train_Annotations , filename+'_'+xmls_List[i])
                # 修改后（直接使用原名）
                xml_save_path = os.path.join(train_Annotations, xmls_List[i])
            ...
            if i <= val_number:
                # image_save_path = os.path.join(test_JPEGImages , filename+'_'+xmls_List[i].split(".xml")[0] + ".jpg")
                image_save_path = os.path.join(test_JPEGImages, xmls_List[i].split(".xml")[0] + ".jpg")
            else:
                # image_save_path = os.path.join(train_JPEGImages , filename+'_'+xmls_List[i].split(".xml")[0] + ".jpg")
                image_save_path = os.path.join(train_JPEGImages , xmls_List[i].split(".xml")[0] + ".jpg")
...
def yolov3_test_add_image_xml(dir,add_path):
    ...
    if len(add_img_list)>0:
        for filename in add_img_list:
            ...
            # 将image复制到test/JPEGimages和image_total中
            if file_path[-4::]==".jpg":
                test_image_path = dir + "test/JPEGImages/" + filename.split(".jpg")[0] + ".jpg"
...
def count_labels_to_csv(src_dir, det_dir):
    ...
    for xml_path in tqdm(xmls_list):
        ...
        #常规下 images和results在同一个目录下；JPEGImages和Annotations在同一目录下
        if xmls_file_name == 'results':
            img_path = os.path.join(file_dir, 'images', img_name)
        elif xmls_file_name == 'Annotations':
            img_path = os.path.join(file_dir, 'JPEGImages', img_name)
...
if __name__ == '__main__':
    src_dir = 'D:\download\datarubbish_train\images_label_split\mask'
    det_dir = 'D:\download\datarubbish_train\label_stats.csv'
